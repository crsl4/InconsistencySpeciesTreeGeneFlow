...
# from one given path
# Claudia April 2015
# modified to not read the dendroscope format, but the networks
# with the gammas.
# this is needed because phylone does not print the parenthetical
# format in the correct way: i.e. they print leaf with major gamma
# Claudia January 2016

#@subfolder = `find /phylonet -type d`;

$path = 'phylonet_noBS/gamma0.1_n10';

foreach(@ARGV){
    if (/\bpath\s*=\s*(.+)/i){$path = $1; }
}

$where = $path."/*phylonet.out"; #needs to match with name given in phylonet.pl
...
foreach(@files){
	...
	$print_next = ($line =~ /Inferred Network #1:/);
    }
    close FHin;
}
close FHlist;
