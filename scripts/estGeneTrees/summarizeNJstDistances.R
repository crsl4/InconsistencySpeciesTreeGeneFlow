# code to call distances.r function to every folder inside NJst/
...
for(g in gamma){
  for(n in nloci){
    ...
    path=paste("./njst/gamma",g,"_n",n,"/",sep="")
    ...
    for(k in 1:length(distvalues)){
      ...
      number=distvalues[k]
      indeces = sapply(data$RFdist,function(x) isTRUE(all.equal(x,number)))
      indeces2 = which(indeces == TRUE)
      datawithdis= data[indeces2,]
      ...
      if(nrow(data4) == 1){
        df = data.frame(tree=data4$tree[1],freq=1,dist=data4$RFdist[1])
