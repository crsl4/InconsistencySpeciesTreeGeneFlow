...
for(g in gamma){
  ...
  for(n in nloci){
    path=paste("./astral/gamma",g,"_n",n,"/",sep="")
  ...
  save(alltable,file=paste(g,"_alltableastral.Rda",sep=""))
  write.table(alltable,file=paste(g,"_alltableastral.txt",sep=""),sep=",",row.names=FALSE)
